...
public class AppFileUtils {
	...
	public static String PATH="D:/iamhere_SSM_study/CarRent/src/main/resources/upload";
	static {
	 ...
	 * 文件下载
	 * @param response
	 * @param path  // images/002.jpg/车车车车车车车车车
	 * @param oldName // “”
	 * @return
	 */
	public static ResponseEntity<Object> downloadFile(HttpServletResponse response, String path, String oldName) {
		...
		System.out.println(file+"-----------------");// D:\\upload\\images\\002
		//5,判断文件是否存在
		if(file.exists()) {
			...
			try {
				try {
					// 如果名字有中文 要处理编码
					oldName = URLEncoder.encode(file.getName(), "UTF-8");
				} catch (Exception e) {
					e.printStackTrace();
				}
				// 把file转成一个bytes
